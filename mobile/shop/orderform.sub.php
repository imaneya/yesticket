...
// wetoz : eximbay
if ($default['de_pg_service'] == 'kcp' && !($default['de_iche_use'] || $default['de_vbank_use'] || $default['de_hp_use'] || $default['de_card_use'] || $default['de_samsung_pay_use'] || $is_kakaopay_use )) { 
    ?>
    <form name="sm_form" method="POST">
        ...
        <input type="hidden" name="payco_direct"   value="">
...
<div id="sod_frm">
    ...
    <section id="sod_frm_pay">
        ...
        if ($default['de_eximbay_use']) { 
            ...
            require_once(G5_MSHOP_PATH.'/eximbay/orderform.enc.php');
            $de_eximbay_pay = unserialize($default['de_eximbay_pay']);
            if (is_array($de_eximbay_pay)) { 
                $z = 0;
                foreach ($de_eximbay_pay as $k) {
                    echo '<li><input type="radio" id="od_settle_eximbay'.$z.'" name="od_settle_case" value="'.$k.'" class="od_settle_eximbay" data-eximbay="1"> <label for="od_settle_eximbay'.$z.'" class="lb_icon card_icon">'.payment_type($k).'</label></li>'.PHP_EOL;
                    $z++;
                }
                $checked = '';
            }
...
$(function() {
    ...
    // eximbay : wetoz : ,.od_settle_eximbay 추가
    $("#od_settle_iche,#od_settle_card,#od_settle_vbank,#od_settle_hp,#od_settle_easy_pay,#od_settle_kakaopay,#od_settle_samsungpay,.od_settle_eximbay").bind("click", function() {
        $("#settle_bank").hide();
