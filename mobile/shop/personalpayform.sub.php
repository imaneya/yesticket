        ...
        if ($default['de_eximbay_use']) { 
            ...
            if (is_array($de_eximbay_pay)) { 
                ...
                foreach ($de_eximbay_pay as $k) {
                    echo '<li><input type="radio" id="pp_settle_eximbay'.$z.'" name="pp_settle_case" value="'.$k.'" class="pp_settle_eximbay" data-eximbay="1"> <label for="pp_settle_eximbay'.$z.'">'.payment_type($k).'</label></li>'.PHP_EOL;
                    $z++;
                }
                $checked = '';
            }
...
function pay_approval()
{
    ...
    var eximbay_use = $(':input:radio[name=pp_settle_case]:checked').attr('data-eximbay');
