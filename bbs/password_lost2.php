...
//아이디 찾기
if($mb_name && $mb_nick){
    $sql = " select mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime from {$g5['member_table']} where mb_email = '$email' AND mb_name = '$mb_name' AND mb_nick = '$mb_nick' ";
    $mb = sql_fetch($sql);
    if (!$mb['mb_id'])
        alert('존재하지 않는 회원입니다.');
        ...
        $content .= addslashes($mb['mb_name'])." ".addslashes($mb['mb_nick'])." 회원님은 ".G5_TIME_YMDHIS." 에 아이디 찾기 요청을 하셨습니다.<br>";
        $content .= '</p>';
        ...
        alert($email.' 메일로 회원아이디가 발송 되었습니다.\\n\\n메일을 확인하여 주십시오.',G5_URL);
}else{
    $sql = " select mb_no, mb_id, mb_name, mb_nick, mb_email, mb_datetime from {$g5['member_table']} where mb_email = '$email' AND mb_id = '$mb_id'";
    $mb = sql_fetch($sql);
    if (!$mb['mb_id'])
        alert('존재하지 않는 회원입니다.');
    else if (is_admin($mb['mb_id']))
