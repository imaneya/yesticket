...
if($cp['it_explan3']) {
    $matchs = get_editor_image($cp['it_explan3'], false);
    ...
    for($i=0;$i<count($matchs[1]);$i++) {
        ...
        if(is_file($srcfile)) {
            ...
            $newfile = preg_replace("/\.([^\.]+)$/", "_".$new_it_id.".\\1", $matchs[1][$i]);
            $cp['it_explan3'] = str_replace($matchs[1][$i], $newfile, $cp['it_explan3']);
        }
    }

    $sql = " update {$g5['g5_shop_item_table']} set it_explan3 = '".addslashes($cp['it_explan3'])."' where it_id = '$new_it_id' ";
    sql_query($sql);
}
