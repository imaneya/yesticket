...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <!-- wetoz : eximbay -->
        <style>
            .eximbay_info_fld th {background-color: #fcfff6;}
        </style>
        ...
        if(!isset($default['de_eximbay_mid'])) {
            sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                            ADD `de_eximbay_use` TINYINT( 4 ) NOT NULL DEFAULT 1 ,
                            ADD `de_eximbay_mid` VARCHAR( 255 ) NOT NULL ,
                            ADD `de_eximbay_key` VARCHAR( 255 ) NOT NULL ,
                            ADD `de_eximbay_cur` CHAR( 3 ) NOT NULL ,
                            ADD `de_eximbay_lan` CHAR( 2 ) NOT NULL ,
                            ADD `de_eximbay_pay` VARCHAR( 255 ) NOT NULL 
                            ; ", true);
            
            sql_query(" ALTER TABLE `{$g5['g5_shop_order_table']}`
                            ADD  `od_eximbay_cur` CHAR( 3 ) NOT NULL
                            ; ", true);
        }
        ?>
        <tr class="pg_info_fld eximbay_info_fld">
            <th scope="row">엑심베이 결제 사용</th>
            <td>
                 <?php echo help("엑심베이 결제기능을 사용하려면 체크하십시오."); ?>
                <label><input type="checkbox" name="de_eximbay_use" value="1" id="de_eximbay_use"<?php echo $default['de_eximbay_use']?' checked':''; ?>> 사용</label>
            </td>
        </tr>
        <tr class="pg_info_fld eximbay_info_fld">
            <th scope="row">엑심베이 상점아이디</th>
            <td>
                <?php echo help("카드결제 승인취소는 고객님 카드 발급사에 반영되기 까지 영업일 기준 3 ~ 4일 정도 소요될 수 있습니다."); ?>
                <input type="text" name="de_eximbay_mid" value='<?php echo $default['de_eximbay_mid']?>' class="frm_input" size="20">
            </td>
        </tr>
        <tr class="pg_info_fld eximbay_info_fld">
            <th scope="row">엑심베이 SECRETKEY</th>
            <td>
                <input type="text" name="de_eximbay_key" value='<?php echo $default['de_eximbay_key']?>' class="frm_input" size="20">
            </td>
        </tr>
        <tr class="pg_info_fld eximbay_info_fld">
            <th scope="row">엑심베이 결제수단</th>
            <td>
                <div style="margin:0 0 10px">
                    <select name="de_eximbay_lan" id="de_eximbay_lan">
                        <option value="">결제창언어</option>
                        <option value="EN" <?php echo (($default['de_eximbay_lan'] == 'EN' || !trim($default['de_eximbay_lan'])) ? 'selected=selected' : '');?>>영문</option>
                        <option value="CN" <?php echo ($default['de_eximbay_lan'] == 'CN' ? 'selected=selected' : '');?>>중문</option>
                        <option value="KR" <?php echo ($default['de_eximbay_lan'] == 'KR' ? 'selected=selected' : '');?>>국문</option>
                    </select>
                    <select name="de_eximbay_cur" id="de_eximbay_cur">
                        <option value="">통화단위</option>
                        <option value="USD" <?php echo (($default['de_eximbay_cur'] == 'USD' || !trim($default['de_eximbay_cur'])) ? 'selected=selected' : '');?>>USD</option>
                        <option value="CNY" <?php echo ($default['de_eximbay_cur'] == 'CNY' ? 'selected=selected' : '');?>>CNY</option>
                        <option value="KRW" <?php echo ($default['de_eximbay_cur'] == 'KRW' ? 'selected=selected' : '');?>>KRW</option>
                    </select>
                </div>
                ...
                if (!is_array($de_eximbay_pay)) { 
                    $de_eximbay_pay = array();
                }
                ?>
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P000" id="de_eximbay_pay1" <?php echo in_array('P000', $de_eximbay_pay) ? ' checked' : '' ; ?>> CreaditCard</label>&nbsp;
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P001" id="de_eximbay_pay2" <?php echo in_array('P001', $de_eximbay_pay) ? ' checked' : '' ; ?>> Paypal</label>&nbsp;
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P003" id="de_eximbay_pay4" <?php echo in_array('P003', $de_eximbay_pay) ? ' checked' : '' ; ?>> Alipay</label>&nbsp;
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P004" id="de_eximbay_pay5" <?php echo in_array('P004', $de_eximbay_pay) ? ' checked' : '' ; ?>> Tenpay</label>&nbsp;
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P141" id="de_eximbay_pay6" <?php echo in_array('P141', $de_eximbay_pay) ? ' checked' : '' ; ?>> WeChat</label>&nbsp;
                <label><input type="checkbox" name="de_eximbay_pay[]" value="P005" id="de_eximbay_pay7" <?php echo in_array('P005', $de_eximbay_pay) ? ' checked' : '' ; ?>> 99Bill</label>
                <?php echo help("엑심베이 결제를 사용할경우 반드시 하나이상 체크해주세요."); ?>
            </td>
        </tr>
        <!-- wetoz : eximbay -->
