...
class item_list
{
    ...
    //function __construct($type=0, $list_skin='', $list_mod='', $list_row='', $img_width='', $img_height='', $ca_id='') {
    function __construct($list_skin='', $list_mod='', $list_row='', $img_width='', $img_height='') {
    ...
    // 특별히 설정하지 않는 경우 상품유형을 사용하는 경우는 쇼핑몰설정 값을 그대로 따릅니다.
    function set_img_size($img_width, $img_height) {
        global $default;
        if ($this->is_mobile) {
    ...
    function run() {
        ...
        if ($this->query) {
            ...
            //언어 추가
            if (get_session($lang)==1) {
              $sql_where .= " and it_cn='1' ";
            } else {
              $sql_where .= " and it_cn='0' ";
            }
...
// 이미지를 얻는다
function get_image($img, $width=0, $height='', $img_id='')
{
    global $g5, $default;
...
// 상품 이미지를 얻는다
function get_it_image($it_id, $width, $height='', $anchor=false, $img_id='', $img_alt='', $is_crop=true)
{
    global $g5;
    ...
    if($filename) {
        ...
        $thumb = thumbnail($filename, $filepath, $filepath, $width, $height, true, $is_crop, 'top', false, $um_value='80/0.5/3');
    }
...
// 상품이미지 썸네일 생성
function get_it_thumbnail($img, $width, $height='', $id='', $is_crop=true)
{
    $str = '';
    ...
    $thumb = thumbnail($filename, $filepath, $filepath, $width, $height, true, $is_crop, 'center', false, $um_value='80/0.5/3');
...
// 사용후기 썸네일 생성
function get_itemuselist_thumbnail($it_id, $contents, $thumb_width, $thumb_height, $is_create=true, $is_crop=true, $crop_mode='center', $is_sharpen=true, $um_value='80/0.5/3')
{
    global $g5, $config;
    $img = $filename = $alt = "";
