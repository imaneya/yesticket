        ...
        if ($default['de_eximbay_use']) { 
            ...
            if (is_array($de_eximbay_pay)) { 
                ...
                foreach ($de_eximbay_pay as $k) {
                    echo '<input type="radio" id="pp_settle_eximbay'.$z.'" name="pp_settle_case" value="'.$k.'" class="pp_settle_eximbay" data-eximbay="1"> <label for="pp_settle_eximbay'.$z.'">'.payment_type($k).'</label>&nbsp;'.PHP_EOL;
                    $z++;
                }
                $checked = '';
            }
